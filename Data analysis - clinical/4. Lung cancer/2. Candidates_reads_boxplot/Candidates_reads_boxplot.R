...
sig_cfRNA<-read.table("../DE_LC_SZDE_vs_NOR_SZBA/DESeq2 significant results-cfRNA-Lung cancer vs. Healthy.txt", header = T)
sig_cfRNA<-sig_cfRNA[sig_cfRNA$baseMean>=10 & sig_cfRNA$log2FoldChange>=1,]
sig_list<-list()
Distribution<-double()
...
plot <- ggplot(subset, aes(x=Type, y=RNA)) + geom_boxplot(aes(fill = Type),outlier.shape=NA)+ scale_fill_manual(values=c("#4E62AB","#D6404E","#9E0142"))+ 
  ...
  labs(x="", y = expression('Cumulative log'[2]*'(RPM+1)'))+ theme_classic()+
