...
meta$group<-ifelse(meta$sample_type=="GC_NXYK","Cases","Controls")
meta$group<-factor(meta$group)
table(meta$sample_type)
table(meta$group)

###Discovery cohort
dis_case<-subset(meta,sample_type=="GC_NXYK")
dis_control<-subset(meta,sample_type=="NOR_SZBA" | sample_type=="BRC_NXYK" | sample_type=="CRC_NXYK" | sample_type=="LC_SZDE" | sample_type=="HCC_NXYK")
...
####Significant RNA
sig_cfRNA<-read.table("../DE_GC_vs._others/DESeq2 candidate results-cfRNA-Cases vs. Controls.txt", header = T)
sig_list<-list()
Distribution<-double()
...
saveRDS(goi,"GOI-Gastric cancer vs. others.rds")
ggplot(coef_filter, aes(x=apply.coef..1.....1..function.x..length.which.x....0...)) + 
  geom_histogram(color="black", fill="gray") + labs(x="Frequency of non-zero coefficient in 100 tests",y="# Gene of interest",title="GC vs. Non-GC")+
...
saveRDS(goi_list,"goi_list_Gastric cancer vs. Non-Gastric cancer.rds")
writeLines(goi_list[["cfRNA"]],"Gastric cancer vs. Non-Gastric cancer.txt")
...
####plot----
saveRDS(ML,"ML_results_Gastric cancer vs. Non-Gastric cancer.rds")
...
ggplot(AUC_Testing_df, aes(x=variable, y= value))+geom_boxplot(outlier.shape = NA)+  geom_jitter(aes(color=method),shape=16, position=position_jitter(0.2),alpha=0.8) + 
  labs(x="", y = "AUC in 100 iterations",title="Gastric cancer vs. Non-Gastric cancer")+ theme_classic()+scale_colour_manual(values=c("#D6404E","#87CFA4","#4E62AB"))+
...
ggsave(paste0("AUC boxplot of different cfRNA-Gastric cancer.pdf"), width = 5, height = 5)
ggsave(paste0("AUC boxplot of different cfRNA-Gastric cancer.png"), width = 5, height = 5, bg="white")
...
discovery_plot<-ggplot(df_discovery_plot, aes(x=id, y=value,color=actual)) + geom_boxplot(outlier.size = 0.1)+ scale_colour_manual(values=c("#4E62AB","#469EB4","#87CFA4","#F0D43A","#F57547","#D6404E"))+
  labs(x=paste0("Study subjects (N=",length(discovery_y),")"), y="Gastric cancer risk score")+ geom_hline(yintercept=0.5, linetype="dashed", color = "black") +
...
discovery_plot
ggsave(paste0("Gastric cancer vs. Non-Gastric cancer-discovery-logistic regression.pdf"), width = 16, height = 3)
ggsave(paste0("Gastric cancer vs. Non-Gastric cancer-discovery-logistic regression.png"), width = 16, height = 3, bg="white")
...
plot<-ggplot(df_median, aes(x=actual, y=df_discovery_median)) + geom_boxplot(outlier.shape=NA)+ geom_jitter(aes(colour = actual),shape=16, position=position_jitter(0.2),size=0.6)+
  scale_colour_manual(values=c("#4E62AB","#469EB4","#87CFA4","#F0D43A","#F57547","#D6404E"))+labs(x="", y="GC risk score")+
...
ggsave(paste0("Gastric cancer median risk score boxplot by group.pdf"), width = 4, height = 4, units="cm")
ggsave(paste0("Gastric cancer median risk score boxplot by group.png"), width = 4, height = 4, units="cm", bg="white")
saveRDS(df_median,'GC-Discovery cohort risk scores.rds')
...
saveRDS(df_validation_median,"Validation_GC_risk_score.rds")
