...
meta$group<-ifelse(meta$Group=="Healthy","Controls","Cases")
...
cfRNA_Volcano<-EnhancedVolcano(results_cfRNA, lab=NA,
                               ...
                               title = "Cancer vs. Cancer-free", subtitle = paste0("loss ",decrease,"  |  gain ",increase), caption = "",
                               pCutoff = 0.1, FCcutoff = 0.8, xlim = c(-4.1,5.4), ylim = c(0,78), pointSize = 1,
                               col = c("black","black","#4E62AB","#D6404E"), colAlpha = 0.9, legendLabels = c("NS", "FC", "FDR", "FDR & FC"))
# cfRNA_Volcano<-cfRNA_Volcano + annotate("text", x = -4, y = 50, label = paste0("loss\n",decrease),size = 5) + 
#   annotate("text", x = 4, y = 50, label = paste0("gain\n",increase),size = 5)+
...
result_seed<-foreach(i=N, .combine="c",.packages=c("tidyverse","ggplot2","ggpubr","rstatix","reshape2")) %dopar% {
  ...
  colnames(df)[1]<-"RNA"
  stat.test <- df %>%
    t_test(RNA ~ Group, paired = FALSE, var.equal = FALSE, p.adjust.method = "none", ref.group = "NOR")%>%
    add_significance("p")%>%
