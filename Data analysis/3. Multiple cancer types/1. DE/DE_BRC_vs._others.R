...
cfRNA_Volcano<-EnhancedVolcano(results_cfRNA, lab=NA,
                               ...
                               title = "BRC vs. Non-BRC", subtitle = paste0("loss ",decrease,"  |  gain ",increase),caption = "",
                               pCutoff = 0.1, FCcutoff = 0.8, xlim = c(-10,5.1), ylim = c(0,62), pointSize = 1, 
                               col = c("black","black","#4E62AB","#D6404E"), colAlpha = 0.9, legendLabels = c("NS", "FC", "FDR", "FDR & FC"))
...
result_seed<-foreach(i=N, .combine="c",.packages=c("tidyverse","ggplot2","ggpubr","rstatix","reshape2")) %dopar% {
  ...
  colnames(df)[1]<-"RNA"
  stat.test <- df %>%
    t_test(RNA ~ Group, paired = FALSE, var.equal = FALSE, p.adjust.method = "none", ref.group = "BRC")%>%
    add_significance("p")%>%
